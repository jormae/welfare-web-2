...
const FormReauditChart = () => {
  ...
  const fetchCharts = async () => {
    try {
      const { data } = await axios.get(`http://localhost:3001/chart/reaudit-chart`)
  ...
  const onSubmit = data => {
    ...
    fetch('http://localhost:3001/chart/reaudit-chart/${an}/${staffName}/${date}', {
      method: 'PUT',
      headers: {
  ...
  return (
    <Card>
      ...
      <CardHeader title='รายการรีออดิทชาร์ต' titleTypographyProps={{ variant: 'h6' }} />
      <Divider sx={{ margin: 0 }} />
      <CardContent>
...
export default FormReauditChart
