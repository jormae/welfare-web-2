...
const FormLoanAgreement = () => {
  ...
  const fetchLoanDetail = async () => {
    const uri = apiConfig.baseURL + `/loans/request/${router.query.nationalId}/${router.query.loanId}`
    console.log(uri)
    try {
  ...
  const fetchLoanAgreementDetail = async () => {
    const uri = apiConfig.baseURL + `/utils/loan-month-range/${router.query.loanId}`
    console.log(uri)
    try {
      const res = await axios.get(uri)
      console.log(res.data[0])
  ...
  const getPlansContent = (loanAgreementDetail) => {
    const content = []
    const approvedAt = loanDetail?.loanApprovedAt

    const startLoanDate = moment(loanDetail?.startLoanDate).add(1, 'months').format('DD/MM/YYYY')
    // const startLoanDate = moment(loanDetail?.startLoanDate).add(1, 'months').format('DD/MM/YYYY')
    const principle = loanDetail?.principle
    const monthlyProfit = loanDetail?.monthlyProfit
    const monthlyPayment = loanDetail?.monthlyPayment
    ...
    for (let i = 1; i <= loanDetail?.loanDurationInMonth; i++) {
      ...
      <TableRow key={item.i}>
        ...
        <TableCell align='center' component='th' scope='row' sx={{ border: '1px solid rgba(224, 224, 224, 1)' }}>
          {moment(loanDetail?.startLoanDate).add(i, 'months').format('MM') + '/' + moment(loanDetail?.startLoanDate).add(543, 'years').format('YYYY')}
        </TableCell>
